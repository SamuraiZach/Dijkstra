...
void Graph:: dijkstra(){ // 5 pts
	...
	for(int i = 0; i < numOfVerts; i++){
		distances[i] = adjMatrix[start][i];//first row of adjMatrix is the distances from start?
		prev[i] = start;
	}
	prev[start] = -1;
	//Step 3:
	...
	/*int count = 0;
	while(count < numOfVerts){
		int distMin = minDistance();
		setDistances(distMin);
		visited[distMin] = true;
		count += 1;
		printInfoSoFar();
	}*/
	for(int k = 0; k<numOfVerts-1;k++){
		int m = minDistance();
		visited[m] = true;
		setDistances(m);
		printInfoSoFar();
...
void Graph::setDistances(int latestVert) { //8 pts
	...
	for(int i = 0; i <numOfVerts; i++){
		if (visited[i] == false &&distances[latestVert] != 10000 && distances[latestVert]+adjMatrix[latestVert][i]<distances[i] ){
...
int Graph::minDistance( ){ //8 pts
	...
	int min = 10000; //a infinity placeholder
	int min_index;
	for(int i = 0; i < numOfVerts; i++){
